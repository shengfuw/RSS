...
to GO
  ...
  print word "Round: " rounds
  print word "frac of nonB "  (believers / count patches)
end

to go-with-embededdness
  loop [
    ask n-of 1 patches with [done? = false] [
       ...
       if ((((-1 * belief) / 8) * (sum [enforcement] of neighbors )) > (strength + costs)) and (belief != compliance)
         [print (sum [enforcement] of neighbors)]
       if ((((-1 * belief) / 8) * (sum [enforcement] of neighbors )) > (strength + costs)) and (belief != compliance)
         [print strength]
       if ((((-1 * belief) / 8) * (sum [enforcement] of neighbors )) > (strength + costs)) and (belief != compliance)
         [print costs]
     ...
     If count patches with [done? = false] = 0
       [set rounds rounds + 1]
     mark-patches
 ...
 to go-without-embededdness
  ...
  ask patches [recolor-patches]
  If count patches with [done? = false] = 0
    [print word "fraction: " fraction]
  If count patches with [done? = false] = 0
    [print (sum [false-believers] of patches)]
  If count patches with [done? = false] = 0
    [print (sum [disbelievers] of patches)]
    ;[do-plots]
  If count patches with [done? = false] = 0
    [stop]
  ]
...
;  if enforcement > 0
;    [set pcolor red]
;  if need-for-enforcement > 0
;    [set pcolor yellow]
;  if disbelievers = 1 and pcolor != red
;    [set pcolor green]
;  if false-believers = 1
;    [set pcolor red]
;  if believers = 1
;    [set pcolor yellow]
  ...
  ;if compliance = 1 and enforcement != 1
  ;  [set pcolor blue]
  ;if compliance = -1 and enforcement != -1
  ;  [set pcolor green]
...
;  if enforcement = 1 and belief = -1
;  [set pcolor red]
;  if enforcement = -1 and belief = -1
;  [set pcolor green]
;  if compliance = 1 and enforcement = 0
;  [set pcolor blue]
;  if compliance = -1 and enforcement = 0
;  [set pcolor white]

to do-plots
  set-current-plot "false-enforcers as fraction of disbelievers"
  plot fraction
end
@#$#@#$#@
GRAPHICS-WINDOW
